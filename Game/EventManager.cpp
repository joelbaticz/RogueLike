...
void EventManager::BroadcastMouse(Message* msg)
{
	...
	if (msg->type == MSG_MOUSEBUTTONUP)
	{
		if (s_NodeHit != nullptr)
		{
			...
			// If a control received the mouse press event, but we release the mouse on another control -> make sure the previous control is unpressed
			((ControlNode*)s_NodeMouseButtonDown)->OnMouseUp(msg);
		}
		s_NodeMouseButtonUp = s_NodeHit;
		
		if (s_NodeMouseButtonUp == s_NodeMouseButtonDown)
		{
			// If the mouse button was released on the same control -> perform click
			if (s_NodeMouseButtonDown != nullptr)
			{
				if (((ControlNode*)s_NodeMouseButtonDown)->GetIsEnabled())
				{
...
void EventManager::BroadcastMouseSubNode(Node* node, Message* msg)
{
	...
	while (node != nullptr)
	{
		//if (msg->m_Type == MSG_MOUSEMOTION)
		{
			...
			{
				Rectangle rectHitBox = Rectangle(0, 0, controlNode->GetWidth(), controlNode->GetHeight());
				Rectangle* pRectHitBox = &rectHitBox;

				//Perform Local to Global coord transformation
				pRectHitBox = RendererFront::ClipToParentRectangle(node, pRectHitBox);

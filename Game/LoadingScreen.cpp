...
{
	LogEngine::Log(LOG_DEBUG, "LoadingScreen", "Constructor called.");
	...
	myTextureSprite->SetX(320);
	myTextureSprite->SetY(236);
	...
	myFireSprite->SetX(400);
	myFireSprite->SetY(50);
...
LoadingScreen::~LoadingScreen()
{
	LogEngine::Log(LOG_DEBUG, "LoadingScreen", "Destructor called.");
...
void LoadingScreen::ProcessMessage(Message* msg)
{
	if (msg->type == MSG_BUTTON_CLICKED)
	{
		...
		{
			RendererFront::SetWindowFullScreen();
		}
